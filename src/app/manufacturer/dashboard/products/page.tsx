    ...
    <div className="p-5 sm:p-6 space-y-6 max-w-5xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={() => setShowAddModal(true)}
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#F15A2B] text-white text-sm font-semibold hover:bg-orange-700 transition-all shadow-sm">
      ...
      <div className="relative">
        ...
        <input value={query} onChange={(e) => setQuery(e.target.value)}
          ...
          className="w-full pl-10 pr-4 py-3 rounded-2xl bg-white border border-gray-200 text-gray-800 placeholder:text-gray-400 focus:border-[#F15A2B] focus:outline-none transition-colors shadow-sm" />
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-12 flex flex-col items-center gap-3 text-center">
          ...
          {!query && (
            <button onClick={() => setShowAddModal(true)}
              className="mt-2 flex items-center gap-2 px-4 py-2 rounded-xl bg-[#F15A2B] text-white text-sm font-semibold hover:bg-orange-700 transition-all">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      ...
                      {product.is_flash_deal && product.approval_status === 'approved' && (
                        <span className="text-xs px-1.5 py-0.5 rounded bg-orange-50 text-[#F15A2B] font-medium">⚡ Flash</span>
                      )}
                    ...
                    {editingPrice === product.id ? (
                      <div className="flex items-center gap-1">
                        ...
                        <input
                          ...
                          autoFocus
                          className="w-20 px-2 py-1 rounded-lg border border-[#F15A2B] text-sm font-['JetBrains_Mono',monospace] font-semibold text-gray-800 focus:outline-none"
                        />
                      ...
                      <button onClick={() => startEditPrice(product)}
                        ...
                        <div>
                          <p className="font-['JetBrains_Mono',monospace] text-sm font-semibold text-[#F15A2B]">{formatINR(product.price_paise)}</p>
                        ...
                        <Pencil className="w-3 h-3 text-gray-300 group-hover:text-[#F15A2B] transition-colors flex-shrink-0" />
                      </button>
                    )}
                  </div>
                    ...
                    {product.approval_status === 'approved' ? (
                      <button onClick={() => toggleFlashDeal(product)} disabled={togglingId === product.id}
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all border ${
                          product.is_flash_deal ? 'bg-orange-50 text-[#F15A2B] border-orange-100 hover:bg-orange-100' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'
                        } disabled:opacity-50`}>
              ...
              <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="sm:col-span-2">
                      ...
                      <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
                        ...
                        className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-800 focus:border-[#F15A2B] focus:outline-none" />
                    </div>

                    <div className="sm:col-span-2">
                      <label className="text-xs font-semibold text-gray-600 block mb-1">Category *</label>
                      <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))}
                        className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-800 focus:border-[#F15A2B] focus:outline-none bg-white">
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                        ...
                        className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-800 focus:border-[#F15A2B] focus:outline-none resize-none" />
                    </div>
                  </div>
                ...
                <div className="px-6 pb-5 pt-3 flex gap-3 flex-shrink-0 border-t border-gray-100">
                  ...
                  <button onClick={submitProduct} disabled={submitting || !form.title || !form.category || !form.price || !form.stock}
                    className="flex-1 py-2.5 rounded-xl bg-[#F15A2B] text-white text-sm font-semibold hover:bg-orange-700 transition-all disabled:opacity-40 flex items-center justify-center gap-2">
